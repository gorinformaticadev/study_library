...
if(is_dir(FCPATH . 'uploads/study_library' . '/')){
  delete_dir(FCPATH . 'uploads/study_library' . '/');
}
if(is_dir(FCPATH . 'uploads/study_library/discussions/attachment' . '/')){
  delete_dir(FCPATH . 'uploads/study_library/discussions/attachment' . '/');
}
...
if ($CI->db->table_exists(db_prefix() . 'study_library_videos_comments')) {
  $CI->db->query('DROP TABLE `' . db_prefix() . 'study_library_videos_comments`');
}
